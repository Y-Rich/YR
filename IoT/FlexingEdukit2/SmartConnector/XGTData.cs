...
namespace SmartConnector.Edukit
{
    public class XGTData
    {
            ...
            else
            {
                ...
                if (ResponseType == XGT_Request_Func.ReadResponse)
                {
                    for (int i = 0; i < BlockCount; i++)
                    {
                        ...
                        Array.Reverse(data);
                        string dataContent = BitConverter.ToString(data, 0).Replace("-", "");

                        XGTAddressData dataValue = new XGTAddressData();

                        dataValue.IntData = Int32.Parse(dataContent, System.Globalization.NumberStyles.HexNumber);
        ...
        /// Read,Write 함수의 finally 구문에서 호출 하도록 작성하시오.
        /// </summary>
        public void MakeDataToBtyeArray()
        {
            try
            {
                NAK_ErrorCotent = string.Empty;
